...
按 `Alt+R`，你正在打的那段英文會被改寫成通順版本，出現在輸入框下方的小卡片裡。按 `Tab` 接受、`Esc` 關閉、繼續打字就自動消失。不用反白、不用右鍵、不用離開輸入框。（想要打字停頓後自動出現建議？選項頁勾一下就有。）
...
預設只在你主動要求時：按 `Alt+R` 改寫游標所在段落。一次按鍵、一次 API 呼叫——單純打字時不會送出任何東西。

在選項頁開啟自動觸發後，打字停頓也會發請求——但要以下條件全過：
...
- 卡片會顯示它正在改寫的原文段落，rich-text 編輯器裡該段落也會加上淡色外框——你永遠知道送出去的是哪一段（也只會送那一段）
- 模型認為你的文字已經夠通順時，卡片會短暫閃一個 ✓，而不是無聲地不出現
- `Alt+R` 永遠有效：略過上面所有條件——包括英文判斷、停用清單、總開關
- 預期中的建議一直沒出現的話，到選項頁開「記錄每次觸發判斷」，原因會印在網頁的 DevTools console
